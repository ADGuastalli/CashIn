...
const updateUserProfile = async (req, res) => {
    const { country, city, birthdate, last_name, user_name, estadocivil, vivecon, hijos, cantidad_de_hijos } = req.body;
    const { userId } = req.user; // El userId viene del token autenticado

    // Validar que todos los datos requeridos estén presentes
    if (!country || !city || !last_name || !user_name || !estadocivil || !vivecon || !hijos || !cantidad_de_hijos) {
        return res.status(400).json({ error: 'Faltan datos requeridos' });
    }
